...
We considered Barnes-Hut algorithm for speeding up the force calculation between vertices and we used simulated annealing in order to achieve better inital graph layout, before we actually run the force-directed algorithm. Using these approaches we im- plemented our own graph visualisation tool which supports real-time interaction with graph during the layout process.
